...
class Billo(toga.App):
    ...
    async def handle_signup(self, name, email, password, confirm_password):
        ...
        """
        try:
            # Double-check validation (in case called directly)
            ...
            # Call auth service
            success, message = await self.auth_service.sign_up(email, password, {"full_name": name})
            
            if not success:
                self.show_error("Signup Failed", message or "Failed to create account. Please try again.")
                return False
                
            # Show success message
            self.main_window.info_dialog("Success", "Account created successfully!")
