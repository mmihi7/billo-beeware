...
def create_terms_screen(app_instance):
    terms_box = toga.Box(style=Pack(direction=COLUMN, margin=10, flex=1))

    title_label = toga.Label(
        "Permissions Required",
        style=Pack(margin=(10, 10), font_size=18, text_align=CENTER, flex=0) # flex=0 for title
    )
    ...
    terms_label = toga.Label(
        terms_text,
        style=Pack(margin=10, text_align=LEFT, flex=1) # flex=1 to take space
    )
    ...
    permissions_status = toga.Label(
        ...
        style=Pack(margin=10, width=200, background_color='#4CAF50', color='white')
    )
